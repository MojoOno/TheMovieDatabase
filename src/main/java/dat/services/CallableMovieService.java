...
public class CallableMovieService
{
    ...
    public void createMovies()
    {
        ...
        List<Future<Movie>> futures = movieDTOs.stream()
                .map(movieDTO -> executor.submit(
                        new MovieTask(movieDTO, sauronDAO, service)))
                .toList();
        ...
        for (Future<Movie> future : futures) {
            try {
                ...
                throw new RuntimeException("Error processing movie task", e);
            }
        }
     ...
     */
    static class MovieTask implements Callable<Movie>
    {
        ...
        public Movie call()
        {
            try
            {
                ...
                // Finally persist-update the movie
                return sauronDAO.update(movie);
            }
            catch (Exception e)
            {
                throw new RuntimeException("Error processing movie task", e);
